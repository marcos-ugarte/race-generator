...
.PHONY: help build test vet fmt lint docker-build run-local

# Default target lists commands.
help: ## Show this help.
	@awk -F':.*## ' '/^[a-zA-Z_-]+:.*## / {printf "  \033[36m%-14s\033[0m %s\n", $$1, $$2}' $(MAKEFILE_LIST)

build: ## Build the race-generator binary into ./race-generator.
...
docker-build: ## Build the self-contained Docker image (IMAGE=$(IMAGE)).
